...
\begin{document}
    ...
    \begin{tikzpicture}[declare function={
	 ...
	 \addplot[mark=text, mark options={fill=blue}, text mark={\tiny $y_{\tiny a}(\alpha) = \varphi'(0) m_1\alpha + \varphi(0)$}] coordinates {(-2.2, 6.4)};
		...
		\addplot[domain=0:0.75, very thick] (x, {i(x)});
	\addplot[mark=*,mark size=1,mark options={fill=black}] coordinates {(0, 0)} node{};
	\addplot[mark=*,mark size=.8,mark options={fill=black},dotted] coordinates {(0, 0) (0, 4)} node{};
