...
$D = \left[ \begin{array}{c} - A_B^{-1} A_N \\ I \end{array}\right]$ then $AD = \left[ A_B, A_N \right] \left[ \begin{array}{c} - A_B^{-1} A_N \\ I \end{array}\right] = -A_B \inv{A_B} A_N + A_N = 0$.
...
\begin{definition}[Cone of feasible directions]
  ...
  Formally, a \textbf{feasible cone} is $F_X(x) = \{ d \in \R^n~:~ \exists \bar{\varepsilon} > 0$ s.t. $x + \varepsilon d \in X, ~ \forall \varepsilon \in [0, \bar{\varepsilon}]\}$.
\end{definition}
...
We are now interested in finding a better characterization of the cone of feasible directions, hence we introduce a new characterization fo the set of constraints $X$.
