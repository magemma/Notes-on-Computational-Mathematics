...
\[
\frac{\partial f}{\partial d} = \ps{\begin{pmatrix}2x e^y\\ x^2 e^y\end{pmatrix}}{\begin{pmatrix} 0\\ 1 \end{pmatrix}} = 
\]
...
\begin{proposition}
Let $f: \R^n \to \R$. 
The partial derivative on a certain direction $\vect{d} \in \R^n$ is the scalar product between the gradient of the function and the direction, formally
\[
\frac{\partial f}{\partial d} = \ps{\nabla f}{ \vect{d}}
\]
\end{proposition}
\todo[inline]{Rivedere prima e seconda componente + parallelo con $e_1, \ldots, e_n$}
...
\begin{definition}[Vector-valued function]
  ...
  Formally, $f: \R^n \to \R^m$, where $f(x) =  \begin{pmatrix} f_1(x)\\f_2(x)\\ \vdots\\ f_m(x)\end{pmatrix} \in \R^m$.
\end{definition}
